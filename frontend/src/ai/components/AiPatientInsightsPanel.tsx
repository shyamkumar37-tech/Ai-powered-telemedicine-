...
export interface AiPatientInsightsPanelProps {
  patientId?: string | number;
    [key: string]: ReturnType<typeof JSON.parse>;
}

export default function AiPatientInsightsPanel({ patientId }: AiPatientInsightsPanelProps) {
  ...
  const [followUp, setFollowUp] = useState<DynamicStateObject | null>(null);
  const [journey, setJourney] = useState<DynamicStateObject | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
    ...
    ]).then(([a, h, f, j]: DynamicStateObject) => {
      if (!active) return;
      ...
      const errorResult = [a, h, f, j].find((item: DynamicStateObject) => item.status === "rejected");
      if (errorResult) {
        ...
        {adherence ? (
          <div className="space-y-2 text-sm text-slate-700">
            ...
            <ul className="list-disc pl-5">
              {(Array.isArray(adherence.nudges) ? adherence.nudges : []).map((item: DynamicStateObject) => (
            ...
            {adherence.rationale?.length ? (
              <ul className="list-disc pl-5 text-xs text-slate-500">
                {adherence.rationale.map((item: DynamicStateObject) => (
                  <li key={item}><LocalizedText value={item} forceTranslate minLength={1} sourceLanguage="auto" /></li>
                ))}
              </ul>
            ) : null}
      ...
      <SectionCard title={<LocalizedText value="Health trend explainer" forceTranslate minLength={1} sourceLanguage="auto" />}>
        {trends ? (
          <div className="space-y-2 text-sm text-slate-700">
            ...
            <ul className="list-disc pl-5">
              {(Array.isArray(trends.keyTrends) ? trends.keyTrends : []).map((item: DynamicStateObject) => (
                <li key={item}><LocalizedText value={item} forceTranslate minLength={1} sourceLanguage="auto" /></li>
              ))}
            </ul>
            {trends.rationale?.length ? (
              <ul className="list-disc pl-5 text-xs text-slate-500">
                {trends.rationale.map((item: DynamicStateObject) => (
                  <li key={item}><LocalizedText value={item} forceTranslate minLength={1} sourceLanguage="auto" /></li>
                ))}
              </ul>
            ) : null}
        ...
        {followUp ? (
          <div className="space-y-2 text-sm text-slate-700">
            ...
            {Array.isArray(followUp.rationale) && followUp.rationale.length ? (
              <ul className="list-disc pl-5 text-xs text-slate-500">
                {followUp.rationale.map((item: DynamicStateObject) => (
                  <li key={item}><LocalizedText value={item} forceTranslate minLength={1} sourceLanguage="auto" /></li>
                ))}
              </ul>
            ) : null}
        ...
        {journey ? (
          <div className="space-y-3 text-sm text-slate-700">
            ...
            <div className="space-y-2">
              {(Array.isArray(journey.steps) ? journey.steps : []).map((step: DynamicStateObject) => (
