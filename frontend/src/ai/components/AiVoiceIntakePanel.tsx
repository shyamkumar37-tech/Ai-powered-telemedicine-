...
export default function AiVoiceIntakePanel({ onApplySummary }: AiVoiceIntakePanelProps) {
  const { t } = useLanguage();
  const recognitionRef = useRef<DynamicState>(null);
  const [sessionId, setSessionId] = useState<DynamicState>("");
  const [stepId, setStepId] = useState<DynamicState>("symptoms");
  const [prompt, setPrompt] = useState<DynamicState>("");
      ...
      {summary ? (
        ...
        <div className="space-y-3">
          ...
          <textarea
            ...
            onChange={(event: DynamicStateObject) => setTranscript(event.target.value)}
          />
