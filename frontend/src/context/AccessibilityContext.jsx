...
const ACCESSIBILITY_CONTEXT_FALLBACK = {
  largeText: false,
  highContrast: false,
...
const AccessibilityContext = createContext(ACCESSIBILITY_CONTEXT_FALLBACK);
...
function normalizeTextForSpeech(text, preferredLanguage = "en") {
  const localized = normalizeWhitespace(text);
  const tokens = SPEECH_FRIENDLY_TOKENS[preferredLanguage] || SPEECH_FRIENDLY_TOKENS.en;

  return tokens.reduce((current, [from, to]) => {
    const pattern = new RegExp(escapeRegExp(from), "gi");
    return current.replace(pattern, to);
  }, localized);
}

function detectContentLanguage(text, preferredLanguage = "en") {
...
function buildSpeechSegments(text, preferredLanguage = "en") {
...
function findBestVoice(voices, languageCode) {
  ...
  const hints = VOICE_NAME_HINTS[baseLanguage] || [];

  return (
    voices.find((voice) => voice.lang?.toLowerCase() === locale && voice.localService)
    || voices.find((voice) => voice.lang?.toLowerCase() === locale)
    || voices.find((voice) => hints.some((hint) => voice.name?.toLowerCase().includes(hint)))
    || voices.find((voice) => voice.lang?.toLowerCase().startsWith(baseLanguage) && voice.localService)
...
export function AccessibilityProvider({ children }) {
  ...
  const [speechSupported, setSpeechSupported] = useState(detectSpeechSupport);
  const [recognitionSupported, setRecognitionSupported] = useState(detectRecognitionSupport);
  ...
  const speak = (text, language = "en", options = {}) => {
    ...
    const speakSegmentAt = (index) => {
      ...
      utterance.lang = SPEECH_LANGUAGE_CODES[segment.language] || SPEECH_LANGUAGE_CODES.en;
      utterance.rate = 0.94;
      utterance.pitch = 1;
  ...
  const announce = (text, language = "en", speakAloud = false) => {
    if (!text) {
      return;
    }
  ...
  const value = useMemo(() => ({
    ...
    toggleLargeText: () => setSettings((current) => ({ ...current, largeText: !current.largeText })),
    toggleHighContrast: () => setSettings((current) => ({ ...current, highContrast: !current.highContrast })),
    toggleScreenReaderMode: () => setSettings((current) => ({ ...current, screenReaderMode: !current.screenReaderMode })),
    resetAccessibility: () => {
