...
import PharmacistPredictiveAnalytics from "../components/pharmacist/PharmacistPredictiveAnalytics";
...
export default function PharmacistInventoryPage() {
  ...
  const [sorting, setSorting] = useState<DynamicStateObject[]>([]);
  ...
  const [formErrors, setFormErrors] = useState<DynamicState>({});
  ...
  const mutation = useMutation({
    ...
    onError: (err: DynamicStateObject) => {
  ...
  const isExpiringSoon = (dateStr: DynamicStateObject) => {
  ...
  const columns = useMemo(() => [
    {
      ...
      header: ({ column }: DynamicStateObject) => (
        <button onClick={() => column.toggleSorting(column.getIsSorted() === "asc")} className="flex items-center gap-1 hover:text-white transition-colors">
          {t("medicine") || "Medicine"}<ArrowUpDown className="w-3 h-3" />
        </button>
      ),
      cell: ({ row }: DynamicStateObject) => (
        <div>
        ...
        <div className="flex flex-col gap-1 items-start">
          ...
          {isExpiringSoon(row.original.expiryDate) && (
            <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-semibold badge-low mt-1">
              <AlertTriangle className="h-3 w-3" /> {t("expiringSoon") || "Expiring Soon"}</span>
          )}
        </div>
      )
    },
    {
      ...
      header: ({ column }: DynamicStateObject) => (
        <button onClick={() => column.toggleSorting(column.getIsSorted() === "asc")} className="flex items-center gap-1 hover:text-white transition-colors">
          {t("available") || "Available"}<ArrowUpDown className="w-3 h-3" />
        </button>
      ),
      cell: ({ row }: DynamicStateObject) => (
    ...
    },
    {
      ...
      header: "Reorder At",
      cell: ({ row }: DynamicStateObject) => <div className="text-sm text-slate-400">{row.original.reorderLevel}</div>
    },
    {
      accessorKey: "expiryDate",
      header: ({ column }: DynamicStateObject) => (
        <button onClick={() => column.toggleSorting(column.getIsSorted() === "asc")} className="flex items-center gap-1 hover:text-white transition-colors">
          {t("expiry") || "Expiry"}<ArrowUpDown className="w-3 h-3" />
        </button>
      ),
      cell: ({ row }: DynamicStateObject) => (
    ...
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      ...
      >
        <div className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">{t("medicineName") || "Medicine Name"}</label>
            <input className={`ph-input ${formErrors.medicineName ? 'border-red-500' : ''}`} placeholder="e.g. Amoxicillin" value={form.medicineName} onChange={(e: DynamicStateObject) => setForm({ ...form, medicineName: e.target.value })} />
            {formErrors.medicineName && <p className="text-red-400 text-xs mt-1">{formErrors.medicineName}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">{t("formulation") || "Formulation"}</label>
            <input className="ph-input" placeholder="e.g. 500mg capsule" value={form.formulation} onChange={(e: DynamicStateObject) => setForm({ ...form, formulation: e.target.value })} />
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">{t("quantityInStock") || "Quantity in Stock"}</label>
              <input className={`ph-input ${formErrors.quantityAvailable ? 'border-red-500' : ''}`} type="number" placeholder="0" value={form.quantityAvailable} onChange={(e: DynamicStateObject) => setForm({ ...form, quantityAvailable: e.target.value })} />
              {formErrors.quantityAvailable && <p className="text-red-400 text-xs mt-1">{formErrors.quantityAvailable}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">{t("reorderThreshold") || "Reorder Threshold"}</label>
              <input className={`ph-input ${formErrors.reorderLevel ? 'border-red-500' : ''}`} type="number" placeholder="0" value={form.reorderLevel} onChange={(e: DynamicStateObject) => setForm({ ...form, reorderLevel: e.target.value })} />
              {formErrors.reorderLevel && <p className="text-red-400 text-xs mt-1">{formErrors.reorderLevel}</p>}
            </div>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">{t("unitContext") || "Unit Context"}</label>
              <input className={`ph-input ${formErrors.unitLabel ? 'border-red-500' : ''}`} placeholder="e.g. tablets, ml" value={form.unitLabel} onChange={(e: DynamicStateObject) => setForm({ ...form, unitLabel: e.target.value })} />
              {formErrors.unitLabel && <p className="text-red-400 text-xs mt-1">{formErrors.unitLabel}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">{t("batchNumber") || "Batch Number"}</label>
              <input className="ph-input" placeholder="e.g. BATCH-123" value={form.batchNumber} onChange={(e: DynamicStateObject) => setForm({ ...form, batchNumber: e.target.value })} />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">{t("expiryDate") || "Expiry Date"}</label>
              <input className="ph-input" type="date" value={form.expiryDate} onChange={(e: DynamicStateObject) => setForm({ ...form, expiryDate: e.target.value })} />
            </div>
          </div>
        </div>
      </PharmacistPremiumCard>
      ...
      {!loading && !error && items.length > 0 && (
        <PharmacistPredictiveAnalytics inventoryItems={items} />
      )}
          ...
          <div className="space-y-4">
            <div className="overflow-x-auto rounded-xl border border-tcd-panel-line">
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  {table.getHeaderGroups().map((headerGroup: DynamicStateObject) => (
                    <tr key={headerGroup.id} className="bg-tcd-panel-2 border-b border-tcd-panel-line">
                      {headerGroup.headers.map((header: DynamicStateObject) => (
                ...
                <tbody className="divide-y divide-tcd-panel-line">
                  {table.getRowModel().rows.map((row: DynamicStateObject) => (
                    <tr key={row.id} className="bg-tcd-panel hover:bg-tcd-panel-2 transition-colors">
                      {row.getVisibleCells().map((cell: DynamicStateObject) => (
