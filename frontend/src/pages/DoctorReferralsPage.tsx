...
export default function DoctorReferralsPage() {
  ...
  const [suggestions, setSuggestions] = useState<DynamicStateObject[]>([]);
  const [referrals, setReferrals] = useState<DynamicStateObject[]>([]);
    ...
    <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr] tcd-animate-in">
          ...
          <button
            ...
            onClick={async () => {
              try {
                ...
                setReferrals((current: DynamicStateObject) => [created, ...current]);
        ...
        <div className="space-y-4">
          <label className="block space-y-2">
            ...
            <select
              ...
              onChange={(e: DynamicStateObject) => setForm({ ...form, patientId: e.target.value })}
            >
              <option value="">{t("selectPatient")}</option>
              {suggestions.map((item: DynamicStateObject) => (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block space-y-2">
              ...
              <input
                ...
                onChange={(e: DynamicStateObject) => setForm({ ...form, specialty: e.target.value })}
              />
            </label>
            <label className="block space-y-2">
              ...
              <select
                ...
                value={form.urgency}
                onChange={(e: DynamicStateObject) => setForm({ ...form, urgency: e.target.value })}
              >
          ...
          <label className="block space-y-2">
            ...
            <input
              ...
              onChange={(e: DynamicStateObject) => setForm({ ...form, targetFacility: e.target.value })}
            />
          </label>
          <label className="block space-y-2">
            ...
            <textarea
              ...
              value={form.reason}
              onChange={(e: DynamicStateObject) => setForm({ ...form, reason: e.target.value })}
            />
          </label>
          <label className="block space-y-2">
            ...
            <textarea
              ...
              onChange={(e: DynamicStateObject) => setForm({ ...form, recommendationNote: e.target.value })}
            />
          </label>
          <label className="block space-y-2">
            ...
            <input
              ...
              onChange={(e: DynamicStateObject) => setForm({ ...form, recommendedDate: e.target.value })}
            />
          </label>
        </div>
      ...
      <div className="space-y-6">
        <PremiumSectionCard title={t("suggestedReferrals")}>
          ...
          <div className="space-y-4">
            {suggestions.map((item: DynamicStateObject) => (
              <div key={`${item.patientId}-${item.specialty}`} className="rounded-xl border border-white/5 bg-[var(--tc-surface-muted)] p-5">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/5 pb-3">
                  ...
                  <span className={`tc-badge ${item.urgency === 'URGENT' ? 'tc-badge tc-badge-danger' : item.urgency === 'PRIORITY' ? 'tc-badge tc-badge-warning' : 'tc-badge tc-badge-neutral'} text-xs px-2.5 py-1`}>{item.urgency}</span>
                </div>
                ...
                <button
                  className="btn-primary btn-secondary mt-4 w-full"
                  type="button"
        ...
        <PremiumSectionCard title={t("existingReferrals")}>
          ...
          <div className="space-y-4">
            {referrals.map((item: DynamicStateObject) => (
              <div key={item.id} className="rounded-xl border border-white/5 bg-[var(--tc-surface-muted)] p-5">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/5 pb-3">
                  <div>
                    <p className="font-semibold text-white tracking-tight">{item.patientName}</p>
                    <p className="text-sm text-slate-400 mt-1">{translateDisplayText(language, item.specialty)}</p>
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`tc-badge ${item.urgency === 'URGENT' ? 'tc-badge tc-badge-danger' : item.urgency === 'PRIORITY' ? 'tc-badge tc-badge-warning' : 'tc-badge tc-badge-neutral'} text-[10px] px-2 py-0.5`}>{item.urgency}</span>
                    <span className="tc-badge tc-badge tc-badge-success text-[10px] px-2 py-0.5">{translateDisplayText(language, item.status)}</span>
                  </div>
                </div>
