  ...
  return (
    <div className="tcd-animate-in space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {Object.entries(form).map(([key, value]: DynamicStateObject) => {
          if (key === "preferredLanguage") {
            return (
              <label key={key} className="block space-y-2">
                <span className="text-sm font-medium text-slate-400">{(fieldLabels as DynamicStateObject)[key]}</span>
                <select className="field" value={value ?? "en"} onChange={(e: DynamicStateObject) => setForm({ ...form, [key]: e.target.value })}>
          ...
          return (
            <label key={key} className={`block space-y-2 ${isTextArea ? 'col-span-1 md:col-span-2' : ''}`}>
              <span className="text-sm font-medium text-slate-400">{(fieldLabels as DynamicStateObject)[key] || key}</span>
              {isTextArea ? (
                <textarea
              ...
              {translatableValueFields.has(key) && value && language !== "en" && (
                <span className="text-xs text-slate-400 block mt-1">
                  {(t("preview") || "Preview")}: <LocalizedText as="span" className="text-teal-400" value={value} />
                </span>
              )}
            </label>
          );
