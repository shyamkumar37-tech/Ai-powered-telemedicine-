...
export default function AccessDeniedPage({ requiredRoles = [] }) {
  ...
  const requiredLabel = requiredRoles.length
    ? requiredRoles.map((role) => roleLabel(normalizeRole(role))).join(", ")
    : "this page";
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-4 py-10">
      <div className="glass-card w-full max-w-xl p-8 text-center">
        ...
        <h1 className="mt-3 text-2xl font-semibold text-ink">{translateUiText("Access restricted")}</h1>
        <p className="mt-3 text-sm text-slate-600">
          {translateUiText(`This page is available only for ${requiredLabel} users.`)}
        </p>
        <p className="mt-2 text-sm text-slate-500">
          {translateUiText(`Signed in as ${roleLabel(normalizedRole)}.`)}
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <button
            ...
            aria-label={translateUiText("Go to my dashboard")}
            data-voice-label={translateUiText("Go to my dashboard")}
          >
            {translateUiText("Go to my dashboard")}
          </button>
          <button
            type="button"
            ...
            aria-label={translateUiText("Switch account")}
            data-voice-label={translateUiText("Switch account")}
          >
            {translateUiText("Switch account")}
          </button>
        </div>
      </div>
    </div>
