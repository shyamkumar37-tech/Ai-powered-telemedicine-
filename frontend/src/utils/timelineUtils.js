...
export function normalizeTimelineDate(value, now = new Date()) {
...
export function formatRelativeTimelineDate(date, now = new Date()) {
  if (!date) {
    return "Time unavailable";
  }
...
export function deriveActor(item) {
  switch (item?.type) {
    case "TRIAGE":
      return "AI Triage";
    case "APPOINTMENT":
...
export function needsAction(item, status) {
...
export function groupTimelineEvents(items) {
  const grouped = [];
  ...
  triageGroups.forEach((value) => grouped.push(value));
  return grouped;
}
