...
export function normalizeAuth(raw) {
  ...
  if (!payload || !payload.sub) {
    if (import.meta.env && import.meta.env.DEV) {
      try { console.debug("[dev] normalizeAuth: invalid jwt payload", { hasToken: Boolean(normalized.token), userId: normalized.userId ?? null }); } catch {}
    }
  ...
  if (!normalized.token || isTokenExpired(normalized.token) || !payloadRole && !normalized.role) {
    if (import.meta.env && import.meta.env.DEV) {
      try { console.debug("[dev] normalizeAuth: token missing/expired or role missing", { hasToken: Boolean(normalized.token), role: normalized.role ?? null, userId: normalized.userId ?? null }); } catch {}
    }
    trackAuthEvent("session-rejected", {
      hasToken: Boolean(normalized.token),
