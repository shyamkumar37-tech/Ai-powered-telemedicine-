...
function getHindiValidationMessage(label: DynamicStateObject, input: DynamicStateObject) {
...
export interface FormFieldProps {
  ...
  leadingIcon?: DynamicState;
  props?: DynamicState;
    [key: string]: ReturnType<typeof JSON.parse>;
}

export default function FormField({ label, error, helperText, leadingIcon, ...props }: FormFieldProps) {
  ...
  const clearCustomValidation = (event: DynamicStateObject) => {
  ...
  return (
    <label className="block space-y-2" htmlFor={fieldId}>
      <span className="text-sm font-semibold text-slate-300">{localizedLabel}</span>
      <div className="relative">
        ...
        <input
          ...
          aria-label={typeof localizedLabel === "string" ? localizedLabel : (props as DynamicStateObject)["aria-label"]}
          data-voice-label={typeof localizedLabel === "string" ? localizedLabel : (props as DynamicStateObject)["data-voice-label"]}
          onInvalid={(event: DynamicStateObject) => {
          ...
          onInput={(event: DynamicStateObject) => {
            clearCustomValidation(event);
            onInput?.(event);
          }}
