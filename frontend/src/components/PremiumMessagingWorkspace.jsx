...
export default function PremiumMessagingWorkspace({ role, title }) {
  const { auth } = useAuth();
  ...
  useWebSocket('/user/queue/messages', handleNewMessage);
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr] tcd-animate-in">
      <PremiumSectionCard title={title}>
            ...
            return (
              <button
                ...
                className={`w-full rounded-xl px-4 py-4 text-left transition ${
                  active ? "bg-teal-400/20 border border-teal-400/30 text-white" : "bg-white/5 border border-white/5 text-slate-300 hover:bg-white/10"
                }`}
              ...
              >
                ...
                <p className={`mt-1 text-sm ${active ? "text-teal-400" : "text-slate-500"}`}>{translateDisplayText(language, contact.descriptor || formatDisplayValue(contact.role))}</p>
              </button>
            );
          })}
        </div>
      ...
      <PremiumSectionCard title={selectedContact ? t("conversationWith").replace("{name}", selectedContact.displayName) : t("composeMessage")}>
        ...
        <textarea
          className="doc-input mt-4 min-h-28 resize-y"
        ...
        <div className="mt-6 space-y-4">
          ...
          {visibleMessages.map((item) => {
            ...
            return (
              <div key={item.id} className={`rounded-xl p-4 border ${outgoing ? "bg-teal-400/10 border-teal-400/20" : "bg-white/5 border-white/10"}`}>
                <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
                  <div>
                    <LocalizedText as="p" className={`font-semibold ${outgoing ? "text-teal-400" : "text-white"}`} value={item.subject} />
                ...
                <LocalizedText as="p" className="mt-3 text-sm text-slate-300 leading-relaxed" value={item.body} />
