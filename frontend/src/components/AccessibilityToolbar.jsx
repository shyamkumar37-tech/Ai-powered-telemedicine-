...
function ToolbarButton({ active = false, onClick, children, ariaLabel, disabled = false }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export default function AccessibilityToolbar({
  ...
  const [open, setOpen] = useState(readInitialToolbarState);
  const [statusMessage, setStatusMessage] = useState("");

  const text = useMemo(() => labels[language] ?? labels.en, [language]);
  const statusCopy = {
