...
function getLocalizedCopy(language) {
  return Object.fromEntries(
    Object.entries(COPY_BASE).map(([key, value]) => [key, translateDisplayText(language, value)])
  );
}

function formatMessage(template, count) {
  return template.replace("{count}", String(count));
}

function describeQueuedItem(item, text) {
...
export default function OfflineQueueBanner() {
  ...
  useEffect(() => subscribeToOfflineQueue((detail) => {
