...
import { translateText, sendCareMessage } from "../../services/telecareService";
...
export default function VideoConsultation({
  ...
  // WebRTC Signaling
  const { sendMessage: sendSignal } = useWebSocket('/user/queue/webrtc', async (signal) => {
  ...
  // Chat Messages
  useWebSocket('/user/queue/messages', (msg) => {
    setMessages(current => [...current, { id: msg.id, sender: patientName, text: msg.body, time: "Now" }]);
  });

  const mixAudioStreams = (localStream, remoteStream) => {
    try {
  ...
  const startWebRTC = async () => {
    ...
    try {
      ...
      pc.onicecandidate = (event) => {
        if (event.candidate) {
          sendSignal('/app/signal', { type: 'candidate', recipientId, data: event.candidate });
        }
      };

      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
      sendSignal('/app/signal', { type: 'offer', recipientId, data: offer });
    } catch (err) {
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 rounded-[1.6rem] bg-slate-950 px-4 py-4 relative">
            ...
            <label className={`inline-flex h-12 w-12 items-center justify-center rounded-full transition cursor-pointer ${isTranscribing ? "bg-amber-500 text-white" : "bg-blue-600 text-white hover:bg-blue-700"}`} title="AI Voice Scribe">
              ...
              <input type="file" accept="audio/*" capture="microphone" className="hidden" onChange={async (e) => {
