...
import com.telecareplus.clinical.FutureCareService;
import com.telecareplus.pharmacy.ReminderService;
...
public class FutureCareServiceImpl implements FutureCareService {
    ...
    public FutureCareDtos.DeteriorationInsightResponse getDeteriorationInsight(Long patientId) {
        ...
        long missedReminders = adherence.missed();
        ...
        int predictedScore = Math.min(
                ...
                30 // Base risk score
                        ...
                        + (missedReminders >= 3 ? 8 : 0)
                        + (activeCaregiverCount > 1 ? -5 : 0)
                        + (adherencePercentage < 75 ? 10 : 0)
        );
        RiskLevel predictedRiskLevel = toRiskLevel(predictedScore);

        List<String> factors = new ArrayList<>();
        if (adherencePercentage < 70) {
            factors.add("Medication adherence is " + adherencePercentage + "%, which is below the safe continuity threshold.");
        }
        if (missedReminders > 0) {
            factors.add(missedReminders + " medication reminders are missed and increasing continuity risk.");
        }
    ...
    public FutureCareDtos.ObservationResponse createObservation(FutureCareDtos.ObservationRequest request) {
        ...
        if (isCritical) {
            applicationEventPublisher.publishEvent(new VitalLoggedEvent(
                    patient.getId(),
                    request.metricName(),
                    request.metricValue(),
                    request.unit(),
    ...
    public List<FutureCareDtos.PopulationInsightResponse> getPopulationInsights(Long doctorId) {
        ...
        long highRisk = patients.stream()
                .filter(patient -> {
                    RiskLevel level = getDeteriorationInsight(patient.getId()).predictedRiskLevel();
