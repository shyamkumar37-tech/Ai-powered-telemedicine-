...
import com.telecareplus.clinical.VitalThresholdService;
...
public class HealthRecordServiceImpl implements HealthRecordService {
    ...
    public HealthDtos.HealthRecordResponse createRecord(HealthDtos.HealthRecordRequest request) {
        ...
        if (request.bloodPressure() != null && vitalThresholdService.isCritical("blood pressure", request.bloodPressure())) {
            applicationEventPublisher.publishEvent(new VitalLoggedEvent(patient.getId(), "blood pressure", request.bloodPressure(), "mmHg", record.getRecordedAt(), true));
        }
        if (request.sugar() != null && request.sugar() > 320) {
            applicationEventPublisher.publishEvent(new VitalLoggedEvent(patient.getId(), "sugar", String.valueOf(request.sugar()), "mg/dL", record.getRecordedAt(), true));
        }
        if (request.spo2() != null && vitalThresholdService.isCritical("spo2", String.valueOf(request.spo2()))) {
            applicationEventPublisher.publishEvent(new VitalLoggedEvent(patient.getId(), "spo2", String.valueOf(request.spo2()), "%", record.getRecordedAt(), true));
        }
        if (request.pulse() != null && vitalThresholdService.isCritical("heart rate", String.valueOf(request.pulse()))) {
            applicationEventPublisher.publishEvent(new VitalLoggedEvent(patient.getId(), "heart rate", String.valueOf(request.pulse()), "bpm", record.getRecordedAt(), true));
        }
        if (request.temperature() != null && vitalThresholdService.isCritical("temperature", String.valueOf(request.temperature()))) {
            applicationEventPublisher.publishEvent(new VitalLoggedEvent(patient.getId(), "temperature", String.valueOf(request.temperature()), "F", record.getRecordedAt(), true));
        }
    ...
    public List<HealthDtos.HealthRecordResponse> getPatientRecords(Long patientId) {
        return healthRecordRepository.findByPatientIdOrderByRecordedAtDesc(patientId).stream().map(this::toHealthResponse).toList();
    }

    private HealthDtos.HealthRecordResponse toHealthResponse(HealthRecord record) {
